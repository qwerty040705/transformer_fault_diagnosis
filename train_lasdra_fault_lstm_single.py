...
# -*- coding: utf-8 -*-
"""
LASDRA 모터 고장 탐지 - BiLSTM 단일 파일 학습 스크립트 (빠르고 정확)
입력: data_storage/link_{L}/fault_dataset_shard_*.npz
특징: SE(3) 잔차(회전벡터3 + 위치3) × (rel, cum, ee) + 시간미분 = 36차원/링크
모델: SE(채널어텐션) + BiLSTM(Stack) + 얕은 Depthwise-Conv 후처리 + Residual
손실: 가중 BCE(불균형) + 온셋 가중 + Total Variation(시간 평활)
성능/안정: CUDA 학습, 특징은 CPU에서 LRU 캐시, BLAS 스레드 1, workers 기본 0
"""

from __future__ import annotations
import os, sys, argparse, glob, random, time, math
from collections import OrderedDict
from typing import Tuple, Dict, Any, List
import numpy as np

# ── 시스템 스레드/메모리 최적화(체감 랙 방지) ─────────────────────────────────────
...
# ── 모델: SE 채널어텐션 + BiLSTM(Stack) + 얕은 DW-Conv 후처리 ───────────────────
...
class DWConvBlock(nn.Module):
    def __init__(self, C: int, dilation: int = 1, dropout: float = 0.1):
        super().__init__()
        groups = math.gcd(C, 8) if C >= 8 else 1  # GroupNorm 안전하게
        self.gn1 = nn.GroupNorm(num_groups=groups, num_channels=C)
        self.dw  = nn.Conv1d(C, C, kernel_size=3, padding=dilation, dilation=dilation, groups=C)
        self.pw  = nn.Conv1d(C, C, kernel_size=1)
        self.gn2 = nn.GroupNorm(num_groups=groups, num_channels=C)
        self.drop= nn.Dropout(dropout)
        self.se  = SEBlock1D(C)
    def forward(self, x):  # (B,C,T)
        h = self.gn1(x)
        h = torch.nn.functional.gelu(self.dw(h))
        h = self.pw(h)
        h = self.gn2(h)
        h = self.se(h)
        h = self.drop(h)
...
class BiLSTMModel(nn.Module):
    """
    입력: (B,T,36) → 출력: (B,T,8)
    핵심: 선형 임베딩 → SE 채널 어텐션 → BiLSTM(Stack) → 얕은 DW-Conv 2블록 → Linear Head
    """
    def __init__(self, in_dim=36, hidden=192, lstm_layers=2, dropout=0.2, conv_post=True):
        super().__init__()
        self.hidden = hidden
        ...
        self.lstm = nn.LSTM(
            input_size=hidden,
            hidden_size=hidden//2,   # bidirectional → 출력 차원 hidden
        ...
        self.conv_post = nn.Sequential(
            DWConvBlock(hidden, dilation=1, dropout=dropout),
            DWConvBlock(hidden, dilation=2, dropout=dropout)
        ) if conv_post else nn.Identity()
    ...
    def forward(self, x):  # (B,T,F)
        h = self.inp(x)               # (B,T,H)
        # SE는 (B,H,T)형태에서 채널 주의
        h = self.se(h.transpose(1,2)).transpose(1,2)  # (B,T,H)
        h_res = h
        h, _ = self.lstm(h)           # (B,T,H)  (bi-directional)
        h = self.ln(h + h_res)        # residual
        h = self.conv_post(h.transpose(1,2)).transpose(1,2)  # (B,T,H)
...
def train_main(args):
    ...
    model = BiLSTMModel(in_dim=36, hidden=args.hidden, lstm_layers=args.lstm_layers,
                        dropout=args.dropout, conv_post=True).to(device)
    ...
    best_bacc = -1.0
    best_path = os.path.join(args.ckpt_dir, "best_lstm.pt")
...
if __name__ == "__main__":
    main()
"""
python3 train_lasdra_fault_lstm_single.py \
